...
namespace LifeSim.Core.Domain.Careers
{
    ...
    {
        // Check if qualified for the lowest tier of the track
        ...
        // Compute current step index from stats — demo: higher iq/eq means higher rank (not using saved rank)
        public static int ComputeStepIndex(Character c, CareerTrack track)
        {
            ...
            for (int i = 0; i < track.ladder.Count; i++)
            {
                ...
                if (c.stats.iq >= step.requiredIQ && c.stats.eq >= step.requiredEQ) 
                    idx = i;
                else break;
            }
            return idx;
